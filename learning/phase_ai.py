"""
learning/phase_ai.py — 10 AI calls orchestration (tách từ phase.py v2).
...
async def run_10_ai_calls_internal(
    ...
    ai1 = await ai_dom_structure(
        _snippet(htmls[0], 10000), urls[0],
        _snippet(htmls[1], 8000),  urls[1],
        ai_limiter,
    )
    all_results["ai1"] = ai1
    if ai1:
    ...
    if n >= 4:
        ...
        ai3 = await ai_stability_check(
            _snippet(htmls[2], 8000), urls[2],
            _snippet(htmls[3], 8000), urls[3],
    ...
    if n >= 7:
        ...
        ai6 = await ai_special_content(_snippet(htmls[6], 8000), urls[6], ai_limiter)
        all_results["ai6"] = ai6
        if ai6:
    ...
    if n >= 8:
        ...
        ai7 = await ai_ads_deepscan(_snippet(htmls[7], 8000), urls[7], ai_limiter)
        all_results["ai7"] = ai7
        if ai7:
