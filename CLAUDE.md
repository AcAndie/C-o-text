...
| **28** | **`writers/` package rename (P1.3)** | **Tên package `writers/` thay cho `output/` theo BLUEPRINT §7 gốc** | **Conflict runtime `output/` dir gitignored — file package sẽ bị ignore. Rename giữ commitable + tránh pollute runtime dir.** |
| **29** | **Writer instance per task (P1.5)** | **`ObsidianWriter` build 1 lần trong `run_novel_task`, pass xuống `_run_scrape_loop` → `scrape_one_chapter`** | **Stateless, no expensive init per chapter. Future writer có thể accumulate state (manifest, image index) — lifecycle per task tự nhiên. Test mock 1 instance dễ.** |
| **30** | **`build_cleaned_chapter` standalone helper (P1.5)** | **Function trong `pipeline/executor.py`, KHÔNG method trên `PipelineRunner`** | **Giữ `runner.run()` return `PipelineContext` unchanged → backward compat caller. Caller (scrape_one_chapter) flexibility build DTO SAU body cleanup (ads filter + strip_nav_edges + title dedup).** |
| **31** | **`_atomic_write_text` async + best-effort cancel cleanup (P1.5)** | **`asyncio.to_thread` + try/except CancelledError → `tmp.unlink()` + re-raise** | **`asyncio.to_thread` không cancel running thread → race window nhỏ (thread tiếp tục sau cancel signal). Accepted: production flow await complete giữa chapters → race thực tế hẹp.** |
| **32** | **`run_config=None` default trong `run_novel_task` (P1.5)** | **Backward compat fallback ObsidianWriter mode** | **Caller cũ (`run_learning_only`, future tests) không cần biết RunConfig. Default safe = obsidian mode.** |
| **33** | **`DomainSessionPool.fetch_bytes` method (P2.2)** | **Tách binary fetch khỏi text `fetch()` — không refactor existing API** | **Reuse per-domain session để giữ TLS fingerprint + cookie consistent. Image fetch cần bytes + Content-Type, không phải decoded text.** |
| **34** | **`_format_element` `is not None` check (P2.4 tangential)** | **Thay `if formatting_rules:` (truthy) bằng `is not None`** | **Empty dict `{}` là valid "use defaults", không nên fall sang plain_text path (silently drop img + bold/italic). Fix while wiring P2.4.** |
| **35** | **Image stage placement = `core/scraper.py` (P2.5)** | **Helper `_apply_image_stage` trong scraper, KHÔNG pipeline/executor.py** | **Scraper has writer (→ run_config, output_dir) + chapter_num + pool. PipelineRunner stays pure (HTML → ctx pieces, no orchestration).** |
| **36** | **AI#image dedicated call (P2.6)** | **New `ai_image_policy` + prompt, KHÔNG extend `ai_ads_deepscan`** | **Single responsibility (ads = post-clean text filter, image = pre-extract structural detect). AI#7 prompt tested + working — mutation risk. Cost +$0.0025/learn negligible.** |
| **37** | **Image fetch failure UX (P2.5)** | **`local_path=None` → body fallback `![alt](original_url)` external link** | **Clickable, not broken. `failed_images` frontmatter list URL fail (P1.4 wired). User aware nhưng vẫn read được.** |
| **38** | **`ingest/web.py` symbolic re-export (P3.3)** | **Re-export `run_novel_task` + thin `scrape_web` wrapper. KHÔNG refactor `main.py` caller** | **Phase 3 = build adapter skeleton (web/epub/txt). Refactor caller defer Phase 6 khi orchestrator route được cả 3. Zero web regression risk hiện tại.** |
| **39** | **`core/orchestrator.py` function-based (P3.6)** | **Functions, không class** | **Stateless dispatcher — class adds no state. `run()` + `run_epub_flow()` đủ.** |
| **40** | **EPUB body fallback via MarkdownFormatter (P3.6)** | **Khi extract chain fail (luôn xảy ra cho EPUB), grab `<body>` tag → format trực tiếp** | **DensityHeuristic 0% hit rate cho EPUB flat `<body><h1><p>` structure (no `<article>`/`<main>` wrapper). Fallback path produces same `CleanedChapter` DTO — writer/downstream agnostic. `selector_used = "epub_body_fallback"` cho debug.** |
| **41** | **Orchestrator bypass PipelineRunner — manual chain compose (P3.6)** | **`ChainExecutor(runner._extract_blocks(), "extract").run(ctx)` direct, không gọi `runner.run()`** | **Spec referenced `runner.run_partial(skip_nav=True)` không tồn tại. Adding = STOP shared logic. EPUB skip Fetch/Nav/Validate anyway → manual compose phù hợp. Orchestrator = composer, không refactor runner.** |
| **42** | **main.py additive EPUB branch — web untouched (P3.6)** | **13-line insertion before `_parse_links_file`. EPUB → orchestrator + return. Web → fall through unchanged** | **Minimal blast radius. Web flow logical zero regression risk. Refactor main.py thành full route-through defer Phase 6.** |
| **43** | **AdsFilter `epub:{slug}` namespace (P3.7)** | **Domain key prefix `epub:` cho EPUB ads keywords trong `data/ads_keywords.json`** | **Tránh collision với web `domain.com` keys. Per-EPUB persistence enables cross-run learning (lần 2 run cùng EPUB → known watermarks filtered Pass 1).** |
| **44** | **AdsFilter EPUB single-pass auto-only — no AI verify (P3.7)** | **`get_candidates_by_frequency(auto_threshold=10, min_count=10)` — auto branch only, AI verify branch skip** | **EPUB không bắt buộc có `ai_limiter`. Threshold 10 occurrences đủ cho real watermark pattern (5-word "Đọc tiếp tại piratesite.com nhé" trigger). Clean EPUB → zero kws → zero false positives. AI verify branch defer.** |
| **45** | **TranslationWriter chunking OFF default (P4.1)** | **`CHUNK_THRESHOLD = 0` — không auto-split chapter** | **Modern LLMs (Gemini 1M, Claude 200K, GPT-4o 128K) handle 30k chars fine. Split adds filename complexity (`0042_part1.txt`). Future configurable via `RunConfig.chunk_threshold` nếu user needs.** |
| **46** | **TranslationWriter/RawWriter title plain first line (P4.1)** | **Title preserved as first line text, KHÔNG strip, KHÔNG `# ` prefix** | **Translator wants context ("Chapter 42: Tiêu đề" + body). Drop title = lose chapter ordering signal trong batch translation.** |
