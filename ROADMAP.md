...
> **Status (2026-05-17):** ✅ **v1.0 SHIPPED.** All 8 phases complete (Phase 0–7). Phase 6 Batch C delivered safe subset only (pure deletion ~0.6%). FlowSpec + image stage extract deferred v1.1 backlog. See [CHANGELOG.md](CHANGELOG.md).
>
...
### P6.1 — Audit codebase post-Phase-5 — ✅ DONE 2026-05-17

- [x] **Bước:**
  1. `wc -l **/*.py` — count current (11,409 LOC across 53 files)
  2. Identify files < 50 dòng → candidate cho merge
  3. Identify unused imports, dead code (pyflakes: 19 hits)
  4. Identify duplicate logic across `ingest/` adapters (80% dup orchestrator)
- [x] **Acceptance:** list candidate files for merge/delete — [docs/AUDIT_PHASE6.md](docs/AUDIT_PHASE6.md)
- [x] **Commit:** `docs: phase 6 codebase audit` (658a492)

### P6.2 — Execute Batch C — ⚠️ PARTIAL 2026-05-17

- [x] **Files động vào:** safe subset shipped
  1. autoflake 17 unused imports (6740293)
  2. drop 22 stale f-prefixes (aef7b0c)
  3. delete `ingest/web.py` unused wrapper (96def6a)
  4. move RawDocument → ingest/types.py (b967435)
  5. inline `_title_from_url`; delete `core/extractor.py` (ef1ec51)
- [x] **Acceptance (partial):**
  - LOC reduction: 11,409 → 11,340 (-69 LOC, **0.6%** — below 5-10% target)
  - All compileall + pyflakes clean
  - Baseline diff: zero (verifiable by inspection, all pure deletion / structural)
- [ ] **Deferred to v1.1:**
  - FlowSpec orchestrator unify (~80 LOC savings) — needs live baseline
  - `_apply_image_stage` extract to `core/image_pipeline/stage.py` (~60 LOC) — STOP §10 (shared logic) + baseline first
- [x] **Commits:** 5 incremental landed.

### P6.3 — Final docs polish — ✅ DONE 2026-05-17

- [x] **Files động vào:**
  - `CLAUDE.md` (Decision Log #50-56, version bump to v1.2)
  - `BLUEPRINT.md` (v1.0 ship status banner, §14 Final Status table, v1.2)
  - `ROADMAP.md` (file này, all phases marked done)
  - `README.md` (full quick start, install, 3 mode usage, troubleshooting hooks, FAQ)
  - `docs/V1_1_BACKLOG.md` (NEW — consolidate defer features w/ rationale)
  - `docs/TROUBLESHOOTING.md` (NEW — 10 common issues + fix)
  - `CHANGELOG.md` (NEW — v1.0.0 entry)
- [x] **Acceptance:**
  - README full-featured (quick start, 3 modes, CLI ref, troubleshooting link, FAQ)
  - 3 governance docs đồng bộ (CLAUDE / BLUEPRINT / ROADMAP cùng v1.2 / cùng phase status)
  - V1_1_BACKLOG.md có rationale cho mỗi defer item
- [x] **Commit:** `docs: finalize v1.0 documentation`

### P6.4 — v1.0 tag + retrospective — ⚠️ USER ACTION

> Last mile reserved for user — needs live network + API quota to capture final baseline + smoke 3 modes.

- [ ] **Bước (cho user):**
  1. Full smoke test: 3 site web + 1 EPUB + 1 TXT × 3 mode
  2. Final baseline snapshot: `python tools/snapshot_baseline.py --label v1.0_final ...`
  3. `git tag v1.0`
  4. Push tag if remote exists: `git push origin v1.0`
- [ ] **Acceptance:**
  - Tag v1.0 ở HEAD
  - Full test pass
- [ ] **Phụ thuộc:** P6.3 ✅
...
| P3 — EPUB Adapter | ✅ DONE 2026-05-16 | Low | Lib mature, structure standard. Dublin Core naming, body fallback for flat EPUB structure, AdsFilter `epub:` namespace |
| P4 — Translation+Raw writers | ✅ DONE 2026-05-17 | Low | TranslationWriter + RawWriter + factory dispatch. Image stage mirror for orchestrator |
| P5 — TXT adapter | ✅ DONE 2026-05-17 | **HIGH (mitigated)** | Exit ramp NOT triggered (Decision #54). VN + EN regex pass. AI fallback + verify. `txt:` AdsFilter namespace |
| P6 — Final cleanup + docs | ⚠️ PARTIAL 2026-05-17 | Low | Audit + safe cleanup done (0.6% LOC). Behavioral refactors (FlowSpec, image stage extract) deferred v1.1. Docs finalized |

**Total estimate:** 7-9 tuần solo dev part-time. **Actual: completed 2026-05-17 (~2 ngày vibe coding aggressive sessions).** Exit ramp Phase 5 NOT triggered.
...
**END ROADMAP v1.2 (v1.0 SHIPPED 2026-05-17)**
